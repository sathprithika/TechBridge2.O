...
function ChatGuide({ visible, onClose }) {
  ...
  const handleSend = async () => {
    try {
      ...
      let response = null;
      if (chat) {
        response = await axios.put(`http://localhost:5000/chat-guide`, {
          id: chat._id,
          email: email,
        ...
        response = await axios.post(`http://localhost:5000/chat-guide`, {
          email: email,
          ...
          <div className="flex flex-col gap-4 overflow-y-auto mb-10">
            {!chat && (
              <div className="flex justify-start">
                <div className="rounded-lg p-2 bg-purple-200 text-left mr-2 w-2/3">{messageGuide.initial}</div>
              </div>
            )}
